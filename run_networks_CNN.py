...
import models.utils.lr_decay as lrd
import models.utils.lr_sched as lr_sched
...
class model():
    ...
    def __init__(self, config, data, test=False):
        ...
        if not self.test_mode:
            ...
            #self.model_optimizer = optim.AdamW(self.model_optim_params_list)
    ...
    def init_models(self, optimizer=True):
        ...
        for key, val in networks_defs.items():
            ...
            '''
            if key=='feat_model':
                self.lr, self.min_lr = optim_params['lr'], optim_params['min_lr']
            '''
            
        self.mixup_fn = None
        if 'mixup' in self.criterions:
            mixup_para = self.criterions['mixup']
            self.mixup_fn = Mixup(mixup_alpha=mixup_para['mixup_alpha'], 
                            cutmix_alpha=mixup_para['cutmix_alpha'],
                            prob=mixup_para['mixup_prob'], 
                            switch_prob=mixup_para['mixup_switch_prob'],
                            mode=mixup_para['mixup_mode'],
                            label_smoothing=mixup_para['smoothing'], 
                            num_classes=self.training_opt['num_classes'])

        self.loss_fn = CADEL_CE_loss()

        cls_num = self.criterions['cls_num'] 
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                if self.mixup_fn is not None:
                    inputs, labels_mixup = self.mixup_fn(inputs, labels)
                    labels_mixup = labels_mixup.cuda()
                ...
                with torch.set_grad_enabled(True):
                    '''
                    lr_sched.adjust_learning_rate2(self.model_optimizer, step/len(self.data['train'])+epoch, 
                                                    lr=self.lr, min_lr=self.min_lr, 
                                                    warmup_epochs=self.training_opt['warmup_epoch'],
                                                    epochs=self.training_opt['num_epochs'])
                    '''
                    self.outputs = self.networks['feat_model'](inputs)
                    # Calculate logits with classifier
                    self.direct_memory_feature = None
                    self.logits = self.networks['classifier'](self.outputs[0])
                    self.outputs = self.outputs + [self.logits]
                    ...
                    for i in range(1, len(self.outputs)):
                        loss_i, weights = self.loss_fn(self.outputs[i], labels_mixup, 
                                                        tempture=self.training_opt['tempture']*epoch/end_epoch,
                                                        weights=weights)
                        self.loss += loss_i
    ...
    def eval(self, phase='val', openset=False, save_feat=False, tao=1, post_hoc=False):
        ...
        for inputs, labels, paths in tqdm(self.data[phase]):
            ...
            with torch.set_grad_enabled(False):
                ...
                if post_hoc:
                    for i in range(1, len(self.outputs)):
                        logits_backup += self.outputs[i] + tao * (self.distribution_target - self.distribution_source)
                else:
                    for i in range(1, len(self.outputs)):
                        logits_backup += self.outputs[i]

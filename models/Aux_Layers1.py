...
class Aux_Layer1(nn.Module):
    def __init__(self, inplanes, num_classes=1000, reduction=8, normalized=False, scale=30, dropout=None):
        ...
        self.preBlock = nn.Sequential(
            nn.Conv2d(inplanes, mid_channel, kernel_size=5, padding=2, groups=1),
            nn.BatchNorm2d(mid_channel),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channel, mid_channel, kernel_size=3, padding=0, groups=32),
            nn.BatchNorm2d(mid_channel),
            nn.ReLU(inplace=True),
            nn.Conv2d(mid_channel, mid_channel, kernel_size=3, padding=0, groups=1),
            nn.BatchNorm2d(mid_channel),
            nn.ReLU(inplace=True),
            nn.Conv2d(2*inplanes, 2*inplanes, kernel_size=3, padding=0),

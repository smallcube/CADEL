...
class My_Layers(nn.Module):
    def __init__(self, block, planes, blocks, stride=1, is_last=False, inplanes=128, groups=1, base_width=64):
...
class ResNext(nn.Module):
    ...
    def __init__(self, block, layers, groups=1, width_per_group=64,
        ...
        self.layer1 = My_Layers(block, 64, layers[0], inplanes=self.inplanes, groups=self.groups,
                                base_width=self.base_width)
        self.inplanes = self.layer1.get_inplanes()
        self.layer2 = My_Layers(block, 128, layers[1], stride=2, inplanes=self.inplanes, groups=self.groups,
                                base_width=self.base_width)
        self.inplanes = self.layer2.get_inplanes()
        self.layer3 = My_Layers(block, 256, layers[2], stride=2, inplanes=self.inplanes, groups=self.groups,
                                base_width=self.base_width)
        self.inplanes = self.layer3.get_inplanes()
        self.layer4 = My_Layers(block, 512, layers[3], stride=2, inplanes=self.inplanes, groups=self.groups,
                                base_width=self.base_width)
    ...
    def forward(self, x, *args):
        ...
        x, _ = self.layer4(x)
        #logits5 = self.aux_layer5(logits5)
        if self.use_modulatedatt:

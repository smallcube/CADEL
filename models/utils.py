...
def weight_name_alignment(model, new_model):
    weight_new = OrderedDict()
    #state_dict = model.state_dict()
    for k in model.state_dict():
        names = k.split(".")
        outside_layer_name = names[0]
        depth_name = names[1]
        inside_layer_name = ""
        if outside_layer_name=="layer1":
            if depth_name=='0':
                inside_layer_name = "layer1"
            elif depth_name=='1':
                inside_layer_name = "layer2"
                depth_name='0'
            else:
                inside_layer_name = "layer2"
                depth_name = "block_2"
        elif outside_layer_name=="layer2":
            depth = int(depth_name)
            if depth==0:
                inside_layer_name = "layer1"
            elif depth>=1 and depth<=3:
                inside_layer_name = "layer1"
                depth_name='block_'+depth_name
            elif depth==4:
                inside_layer_name = "layer2"
                depth_name = "0"
            else:
                inside_layer_name = "layer2"
                depth_name = "block_"+depth_name
        elif outside_layer_name=="layer3":
            depth = int(depth_name)
            if depth>=0 and depth<=17:
                inside_layer_name = "layer1"
                depth_name = "block_"+depth_name
            elif depth==18:
                inside_layer_name = "layer2"
                ...
                inside_layer_name = "layer2"
                depth_name = "block_"+depth_name
        elif outside_layer_name=="layer4":
            if depth_name=='0':
                inside_layer_name = "layer1"
            elif depth_name=='1':
                inside_layer_name = "layer2"
        ...
        #assemble new key
        #print("inside_layer_name=", inside_layer_name)
        new_key = outside_layer_name + "." + inside_layer_name + "." + depth_name
        for i in range(2, len(names)):
            new_key = new_key + "." + names[i]
        
        weight_new[new_key] = model.state_dict()[k]
        
    new_model.load_state_dict(weight_new, strict=False)
    return new_model

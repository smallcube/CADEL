...
class Aux_Layer1(nn.Module):
    def __init__(self, inplanes, num_classes=1000, base_width=64, groups=32, reduction=8, normalized=False, scale=30,
        ...
        # print("width=", width, "    inplane=",2*inplanes)
        self.preBlock = nn.Sequential(
            ...
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True),
            nn.Conv2d(width, mid_channel, kernel_size=3, padding=1, groups=1),
...
class Aux_Layer2(nn.Module):
    def __init__(self, inplanes, num_classes=1000, base_width=64, groups=32, reduction=8, normalized=False, scale=30,
        ...
        self.preBlock = nn.Sequential(
            ...
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True),
            nn.Conv2d(width, width, kernel_size=3, padding=1, groups=1),
            nn.BatchNorm2d(width),
            nn.ReLU(inplace=True),
            nn.Conv2d(width, mid_channel, kernel_size=3, padding=1),
            nn.BatchNorm2d(mid_channel),
            # nn.ReLU(inplace=True)
        )

...
#modify the learning rate adjustment strategy
...
class model():
    ...
    def __init__(self, config, data, test=False):
        ...
        self.criterions = self.config['criterions']
        self.data = data
        self.test_mode = test
        ...
        #self.model_optimizer_scheduler = self.init_optimizers(self.optimizer)
    ...
    def init_models(self, optimizer=True):
        ...
        for key, val in networks_defs.items():
            ...
            model_without_ddp = self.networks[key].module
            ...
            param_groups = lrd.param_groups_lrd(model_without_ddp, optim_params['weight_decay'],
                no_weight_decay_list=model_without_ddp.no_weight_decay(),
                layer_decay=optim_params['layer_decay']
            )
            
            #effective_batch_size = self.training_opt['batch_size']*self.training_opt['accum_iter']
            self.lr = optim_params['lr']    #*effective_batch_size/256
            self.min_lr = optim_params['min_lr']

            self.optimizer = torch.optim.AdamW(param_groups, lr=self.lr, betas=(0.9, optim_params['adamW2']))
        ...
        #print(self.networks['feat_model'])
    ...
    def train(self):
        ...
        for epoch in range(1, end_epoch + 1):
            ...
            for step, (inputs, labels, indexes) in enumerate(self.data['train']):
                ...
                #if step % self.training_opt['accum_iter']==0:
                lr_sched.adjust_learning_rate2(self.optimizer, step/len(self.data['train'])+epoch, 
                                                lr=self.lr, min_lr=self.min_lr, 
                                                warmup_epochs=self.training_opt['warmup_epoch'],
                                                epochs=self.training_opt['num_epochs'])
                
                # If on training phase, enable gradients
                with torch.set_grad_enabled(True):

                    # If training, forward with loss, and no top 5 accuracy calculation
                    self.logits = self.networks['feat_model'](inputs)
                    ...
                    for i in range(len(self.logits)):
                        ...
                        loss_i, weight1, weight2 = Ensemble_BCE_loss_v3(pred=self.logits[i], target=labels, target2=y_b, lam=lam,
                        ...
                        '''
                        loss_i, weight = CADEL_CE_loss_v3(pred=self.logits[i], target=labels, target2=y_b, lam=lam,
                                                        weights=weight,
                                                        tempture=1.0*self.epoch/end_epoch)
                        self.loss = self.loss + loss_i
                        '''
                        logits_backup = logits_backup + self.logits[i]
                    del weight1, weight2
                    ...
                    for i in range(len(self.logits)):
                        ...
                        loss_i, weights = kd_loss(pred=self.logits[i], target=labels, target2=y_b, lam=lam,
                                                    weights=weights,
                                                    alpha = self.training_opt['alpha'],
                                                    tempture=self.training_opt['tempture'])
                        
                        loss_i, weights = CADEL_CE_loss(pred=self.logits[i], target=labels, target2=y_b, lam=lam,
                                                    weights=weights,
                                                    alpha=self.training_opt['alpha'],
                                                    gamma=self.training_opt['gamma'],
                                                    down_weight=(self.epoch<=end_epoch//2))
                        self.loss = self.loss + loss_i
                        logits_backup = logits_backup + self.logits[i]
                    del weights
                    ...
                    #print("step=", step)
                    self.loss_scaler(self.loss, self.optimizer, 
                                    parameters=self.networks['feat_model'].parameters(), 
                                    create_graph=False, update_grad=True)
                    # Zero out optimizer gradients
                    self.optimizer.zero_grad()
                    loss_display = self.loss.item()
                    
                    # Tracking predictions
                    _, preds = torch.max(self.logits_ensemble, 1)
                    total_preds.append(torch2numpy(preds))
                    total_labels.append(torch2numpy(labels))
                    #print('preds.shape=', preds.shape, "   label.=", labels.shape)
                    ...
                    if step % self.training_opt['display_step'] == 0:
                        ...
                        minibatch_loss_total = loss_display
    ...
    def eval(self, phase='val', openset=False, save_feat=False, tao=1):
        ...
        for inputs, labels, paths in tqdm(self.data[phase]):
            ...
            with torch.set_grad_enabled(False):
                ...
                temp_logits = self.networks['feat_model'](inputs)
                logits_backup = 0
                for i in range(0, len(temp_logits)):
                    logits_backup += temp_logits[i] + tao * (self.distribution_target - self.distribution_source)
                #logits_backup /= len(temp_logits) - 1
